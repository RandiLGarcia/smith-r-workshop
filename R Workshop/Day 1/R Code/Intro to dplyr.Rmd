...
We'll use the package `dplyr`. The `dplyr` package contains the functions for all of the data cleaning varbs: `filter()`, `mutate()`, `rename()`, `arrange()`, `select()`, `summarize()`, and `group_by()`. You can find a cheat sheet for `dplyr` [here](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf).
...
We can save this new data set in our files as a csv. Note that the file path conventions are differnt for Windows machines. If copying and pasting from properties, you will need to change backslashes to forward slashes. 

```{r}
write.csv(menOnly, 
  file = "/Users/randigarcia/Desktop/Three-day-workshop/R Workshop/Data/men.csv")
```
...
How would you get the frequncies on this vairable?
...
We now seen our first piplines, using `group_by()`. Now we can make a pipeline of many of the commands I did above. The last thing I do is drop useless `gender` variable, because the resulting dataset if all men.
